...
public class MishnaYomitNoatebook extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonSaveToCloud.setOnClickListener(v -> {
            ...
            String notebookId = "notebook2"; // כל מחברת שונה תקבל שם אחר
    ...
    private void saveNoteToCloud() {
        ...
        databaseReference.child(noteId).setValue(noteText)
                .addOnSuccessListener(aVoid -> {
                    ...
                    Toast.makeText(MishnaYomitNoatebook.this, "ההערה נשמרה בענן!", Toast.LENGTH_SHORT).show();
                })
                .addOnFailureListener(e -> {
                    progressDialog.dismiss();
                    Toast.makeText(MishnaYomitNoatebook.this, "שגיאה בשמירת ההערה בענן", Toast.LENGTH_SHORT).show();
                });
    }

    private void deleteNoteFromCloud() {
        databaseReference.removeValue()
                .addOnSuccessListener(aVoid -> Toast.makeText(MishnaYomitNoatebook.this, "כל ההערות נמחקו מהענן!", Toast.LENGTH_SHORT).show())
                .addOnFailureListener(e -> Toast.makeText(MishnaYomitNoatebook.this, "שגיאה במחיקת ההערות מהענן", Toast.LENGTH_SHORT).show());
    }
}
